...
trait Metable
{
    ...
    public function scopeWhereMeta($query, $key, $value, $alias = null)
    {
        ...
        $qry = $query->join($this->getMetaTable() . ' AS ' . $alias, $this->getQualifiedKeyName(), '=', $alias . '.' . $this->getMetaKeyName())
            ->where('key', '=', $key)
            ->where('value', '=', $value);

        if( $this->getMetaTypeKey () != "" ){
            $qry = $qry->where( $this->getMetaTypeKey (),'=', $this->getMetaTypeTarget() );
        }

        return $qry->select($this->getTable() . '.*');
    }
    ...
    public function metas()
    {
        ...
        if( $this->getMetaTypeKey () != "" ){
            $model = $model->where( $this->getMetaTypeKey (),'=', $this->getMetaTypeTarget() );
        }
    ...
    protected function getMetaData()
    {
        if (!isset($this->metaLoaded)) {
            ...
            if ($this->exists) {
                ...
                if( $this->getMetaTypeKey () != "" ){
                    $objects = $objects->where( $this->getMetaTypeKey (),'=', $this->getMetaTypeTarget() );
                }
    ...
    {
        return isset($this->metaTypeKey) ? $this->metaTypeKey : "";
    }
    ...
    {
        return isset($this->metaTypeTarget) ? $this->metaTypeTarget : "";
    }
